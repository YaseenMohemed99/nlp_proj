...
class TruncatedDataset(torch.utils.data.Dataset):
    def __init__(self, texts, tokenizer, normalization="raw"):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(tqdm(train_loader)):
            ...
            with torch.cuda.amp.autocast(enabled=args.fp16):
                outputs = model(input_ids=inputs, attention_mask=masks, labels=labels, use_cache=False)
...
def main():
    ...
    train_dataset = TruncatedDataset(train_texts, tokenizer, normalization=args.normalization)
    val_dataset = TruncatedDataset(val_texts, tokenizer, normalization=args.normalization)
